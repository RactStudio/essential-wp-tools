...
if (!function_exists('ewpt_enable_maintenance_mode_rsmhr')) {
	// Function to render the maintenance mode page
	function ewpt_enable_maintenance_mode_rsmhr() {
		// Styles and Scripts
		...
		$mm_current_url = $protocol . sanitize_text_field($_SERVER['HTTP_HOST']) . sanitize_text_field($_SERVER['REQUEST_URI']);
...
<head>
	...
	<link rel="canonical" href="<?php echo esc_url($mm_current_url); ?>" />
	<!-- Styles and Scripts -->
	<link rel='stylesheet' id='ewpt-mm-styles' href='<?php echo esc_url($styles_url); ?>' media='all' />
	<style>
		:root {
	...
	/**
	<!--
	<script src="<?php echo esc_url($scripts_url); ?>" id="ewpt-mm-scripts"></script>
	-->
	**/
	?>
    ...
    switch ($condition_value) {
        case 'except_admin':
            if (!current_user_can('administrator')) {
                add_action('wp', 'ewpt_enable_maintenance_mode_rsmhr', 1); // Except Admin
            }
            break;
        case 'except_admin_editor':
            if (!current_user_can('administrator') && !current_user_can('editor')) {
                add_action('wp', 'ewpt_enable_maintenance_mode_rsmhr', 1); // Except Admin, & Editor
            }
            break;
        case 'except_admin_editor_author':
            if (!current_user_can('administrator') && !current_user_can('editor') && !current_user_can('author')) {
                add_action('wp', 'ewpt_enable_maintenance_mode_rsmhr', 1); // Except Admin, Editor, & Author
            }
            break;
        case 'only_author':
            if (current_user_can('author')) {
                add_action('wp', 'ewpt_enable_maintenance_mode_rsmhr', 1); // Only Author
            }
            break;
        case 'only_contributor':
            if (current_user_can('contributor')) {
                add_action('wp', 'ewpt_enable_maintenance_mode_rsmhr', 1); // Only Contributor
            }
            break;
        case 'only_subscriber':
            if (current_user_can('subscriber')) {
                add_action('wp', 'ewpt_enable_maintenance_mode_rsmhr', 1); // Only Subscriber
            }
            break;
        case 'only_logged_in_users':
            if (is_user_logged_in()) {
                add_action('wp', 'ewpt_enable_maintenance_mode_rsmhr', 1); // Only Logged-in Users
            }
            break;
        case 'only_non_logged_in_users':
            if (!is_user_logged_in()) {
                add_action('wp', 'ewpt_enable_maintenance_mode_rsmhr', 1); // Only Non Logged-in Users
            }
            break;
        case 'all_users':
            add_action('wp', 'ewpt_enable_maintenance_mode_rsmhr', 1); // All Users
            break;
        default:
