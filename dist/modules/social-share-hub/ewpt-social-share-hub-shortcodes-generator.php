...
use EWPT\Modules\SocialShareHub\sshub as sshub;


add_shortcode( 'ewpt_social_share_hub', function ($atts) {
	...
	$total_ssh_slots = intval(get_option('enable_total_share_buttons_counter', 5));
	if ($total_ssh_slots < 1) {
		$total_ssh_slots = 1;
	} elseif ($total_ssh_slots > 20) {
		$total_ssh_slots = 20;
	}
	...
	for ($i = 1; $i <= $total_ssh_slots; $i++) {
		...
		$options[$i] = array(
			...
			'ssh_border_radius' => intval($icons_area_border_radius),
		);
	}
