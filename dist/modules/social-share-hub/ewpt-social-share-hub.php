...
add_action( 'init', function () {
	...
	$total_share_buttons_slots = intval(get_option('enable_total_share_buttons_counter', 5));
	if ($total_share_buttons_slots < 1) {
		$total_share_buttons_slots = 1;
	} elseif ($total_share_buttons_slots > 20) {
		$total_share_buttons_slots = 20;
	}
...
if (!function_exists('ewpt_social_share_hub_settings_page')) {
function ewpt_social_share_hub_settings_page() {
	...
	$ssh_i = intval(get_option('enable_total_share_buttons_counter', 5));
	if ($ssh_i < 1) {
		$ssh_i = 1;
	} elseif ($ssh_i > 20) {
		$ssh_i = 20;
...
?>
    ...
    <div id="ewpt-page-main" class="wrap ewpt-page-main">
		...
		<div id="ewpt-page-body" class="ewpt-page-body">
			...
			<form id="<?php echo sanitize_html_class(strtolower(EWPT_SHORT_SLUG)); ?>-form" name="<?php echo sanitize_html_class(strtolower(EWPT_SHORT_SLUG)); ?>-form" method="post" action="options.php">
					...
					<div class="tab-pane">
						...
						<table class="form-table ewpt-form ewpt-form-border-bottom">
							<tr valign="top">
								...
								<td>
									<label>
										<input type="checkbox" name="ewpt_disable_all_social_share_hub_options" value="1" <?php checked(get_option('ewpt_disable_all_social_share_hub_options', 0)); ?> />
										Disable
									</label>
								</td>
								<td>
						...
						<table class="form-table ewpt-form ewpt-no-bottom-border">
							...
							<tr valign="top">
								<th scope="row">Total Social Share Slots</th>
								<?php
									$share_buttons_counter = intval(get_option('enable_total_share_buttons_counter', 5));
									if ($share_buttons_counter < 1) {
										$share_buttons_counter = 1;
									} elseif ($share_buttons_counter > 20) {
										$share_buttons_counter = 20;
									}
								?>
								<td>
									<input type="number" name="enable_total_share_buttons_counter" value="<?php echo intval($share_buttons_counter); ?>" />
								</td>
								<td>
									<div class='ewpt-info-blue'>
										Decreasing the Social Share slot's does not clear saved data.<br/>
										<strong>Note: Default: 5, Minimum: 1, and Maximum: 20.</strong>
									</div>
								</td>
							</tr>
							...
							<tr valign="top">
								<th scope="row">All Social Share Shortcodes</th>
								<td>
									<label>
				...
				<?php for ($i = 1; $i <= $ssh_i; $i++) : ?>
					<div id="ssh-<?php echo intval($i); ?>" class="tab-content" style="display: none;">
						<div class="tab-pane">
							...
							<table class="form-table ewpt-form ewpt-no-bottom-border">
								...
								<tr valign="top">
									...
									<td>
										<label>
											<input type="checkbox" name="share_buttons_<?php echo intval($i); ?>_slot" value="1" <?php checked(get_option("share_buttons_{$i}_slot", 0)); ?> />
											Enable
										</label>
									</td>
								</tr>
							...
							<table class="form-table ewpt-form">
								...
								<tr valign="top">
									...
									<?php
										$ssh_icons_sizes = intval(get_option("share_buttons_{$i}_icons_sizes", 32));
										if ($ssh_icons_sizes < 12) {
											$ssh_icons_sizes = 12;
										} elseif ($ssh_icons_sizes > 128) {
											$ssh_icons_sizes = 128;
										}
									?>
									<td>
										<label>
											<input type="number" name="share_buttons_<?php echo intval($i); ?>_icons_sizes" value="<?php echo intval($ssh_icons_sizes); ?>" />
											PX
										</label>
									</td>
									<td>
								...
								<tr valign="top">
									...
									<?php
										$ssh_icons_border_radius = intval(get_option("share_buttons_{$i}_icons_border_radius", 12));
										if ($ssh_icons_border_radius < 0) {
											$ssh_icons_border_radius = 0;
										} elseif ($ssh_icons_border_radius > 128) {
											$ssh_icons_border_radius = 128;
										}
									?>
									<td>
										<label>
											<input type="number" name="share_buttons_<?php echo intval($i); ?>_icons_border_radius" value="<?php echo intval($ssh_icons_border_radius); ?>" />
											PX
										</label>
									</td>
									<td>
							...
							<table class="form-table ewpt-form ewpt-no-bottom-border">
								...
								<tr valign="top">
									...
									<?php
										$icons_area_border_radius = intval(get_option("share_buttons_{$i}_border_radius", 5));
										if ($icons_area_border_radius < 0) {
											$icons_area_border_radius = 0;
										} elseif ($icons_area_border_radius > 128) {
											$icons_area_border_radius = 128;
										}
									?>
									<td>
										<label>
											<input type="number" name="share_buttons_<?php echo intval($i); ?>_border_radius" value="<?php echo intval($icons_area_border_radius); ?>" />
											PX
										</label>
									</td>
									<td>
										<div class='ewpt-info-blue'>
											Recommended radius: min: 0 to max: 128. Default radius: 5 (in pixels)
										</div>
									</td>
								</tr>
								...
								<tr valign="top">
									...
									<?php
										$icons_area_padding = intval(get_option("share_buttons_{$i}_padding", 5));
										if ($icons_area_padding < 0) {
											$icons_area_padding = 0;
										} elseif ($icons_area_padding > 128) {
											$icons_area_padding = 128;
										}
									?>
									<td>
										<label>
											<input type="number" name="share_buttons_<?php echo intval($i); ?>_padding" value="<?php echo intval($icons_area_padding); ?>" />
											PX
										</label>
									</td>
									<td>
										<div class='ewpt-info-blue'>
											Recommended padding: min: 0 to max: 128. Default padding: 5 (in pixels)
										</div>
									</td>
								</tr>
							...
							<table class="form-table ewpt-form ewpt-no-bottom-border">
								...
								<?php if ( !empty($ewpt_post_types) && (get_option('enable_ewpt_share_buttons_custom_post_types', 0) == 1)) {  ?>
									...
									<?php foreach ($ewpt_post_types  as $post_type) { ?>
											<td>
												<label>
													<input type="checkbox" name="share_buttons_<?php echo intval($i); ?>_place_<?php echo sanitize_html_class($post_type); ?>" value="1" <?php checked(get_option("share_buttons_{$i}_place_{$post_type}", 0)); ?> />
		...
		<div id="ewpt-page-footer" class="ewpt-page-footer">
			...
			//if (isset($_GET['page']) && $_GET['page'] === 'ewpt-social-share-hub') {
				...
				wp_enqueue_style('ssh-style', plugin_dir_url(__FILE__) . 'assets/css/style.min.css', array(), '1.0.0');
				wp_enqueue_script('ssh-jquery-ui-script', plugin_dir_url(__FILE__) . 'assets/js/fieldChooser.min.js', array('jquery-ui-core', 'jquery-ui-sortable'), '1.0.6', true);
			//}
			?>
			...
			(function($) {
				$(document).ready(function () {
						...
						mutationsList.forEach(function(mutation) {
							if (mutation.type === 'childList') {
								mutation.addedNodes.forEach(function(node) {
									...
									if (optionName && !$target.find('input[type="hidden"]').length) {
					...
					// Start observing the selected buttons area
					observer.observe(document.getElementById("share_buttons_<?php echo intval($i); ?>_selected_buttons"), observerConfig);
	...
	include plugin_dir_path(__FILE__) . 'ewpt-social-share-hub-actions.php';
